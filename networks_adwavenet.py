...
class AD_WaveNet(nn.Module):
    def __init__(self, steps=4, layers=4, channels=32, klvl=3, mode='dct', dnlayers=3):
        ...
        for ii in range(4):
            self.encoder_xd_layers.append(TransformerBlock(dim=3, num_heads=3, ffn_expansion_factor=2.66, bias=False, LayerNorm_type='WithBias'))
...
model = AD_WaveNet(steps=4, layers=2, channels=32, klvl=2,
                     mode="wavelet", dnlayers=4).to(device_nv)
model.eval()
