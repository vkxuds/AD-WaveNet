...
def main():
    ...
    input_dir = 'data/dataset_tomowave/fang_24_30_zero/test'
    target_dir = 'data/dataset_tomowave/fang24_true/test'
   ...
   # psnr_sum=0
    ...
    while epoch <= opt.epochs:
        ...
        '''if torch.isnan(Loss) or torch.isinf(Loss):
            if epoch > 0:
                ...
                model.load_state_dict(torch.load(os.path.join(opt.outf, 'net_AD_WaveNet_epoch_{}.pth'.format(epoch-1))))
            else:
                net = AD_WaveNet(steps=opt.num_of_steps, layers=opt.num_of_layers, channels=opt.num_of_channels, klvl=opt.lvl,
    ...
    print("best_epoch=",best_epoch,".and best_psnr = ",best_psnr)
