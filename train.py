    ...
    while epoch <= opt.epochs:
        with tqdm(loader_train, unit='batch') as tepoch:
            ...
            for data in tepoch:
                ...
                while loss_edge >= 0.5*loss_MSELoss:
                ...
                new_loss = [loss_fft.item(),loss_edge.item()]

                weights = dynamic_weight_average(previous_loss, new_loss)
                #weights = [0.5 * w for w in weights]

                loss = loss_MSELoss + weights[0]*loss_fft+weights[1]*loss_edge
                ...
                if i % 400 == 0:
                    ...
                    print("epoch",epoch,"；best_I：",i," val_psnr is ",val_psnr,"best val psnr",best_val_psnr)
                    if  val_psnr>=best_val_psnr:
                        best_val_psnr = val_psnr
                        torch.save(model.state_dict(), os.path.join(opt.outf, 'net_AD_WaveNet_best_val_epoch_{}_i_{}_{:.3f}.pth'.format(epoch,i,best_val_psnr)))

                    print(loss_MSELoss,'mse')
                    print(weights[1]*loss_edge,'edge')
                    print(weights[0]*loss_fft,'fft')
